...
namespace nubors\filemanager\models;
...
use nubors\filemanager\models\Owners;
...
class Mediafile extends ActiveRecord
{
    ...
    public function saveUploadedFile(array $routes, $rename = false)
    {
        ...
        do{
            if($counter==0)
                $filename = Inflector::slug($this->file->baseName). '_'. uniqid().'.'. $this->file->extension;
            else{
                //if we don't want to rename we finish the call here
                if($rename == false)
                    return false;
                $filename = Inflector::slug($this->file->baseName). '_'. uniqid(). $counter.'.'. $this->file->extension;
            }
